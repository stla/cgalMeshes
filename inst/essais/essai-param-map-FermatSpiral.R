...
rmesh <- parametricMesh(
  ...
  periodic = c(TRUE, FALSE), nu = 1024, nv = 1024, clean = TRUE
)

mesh <- cgalMesh$new(rmesh)

# compute the discrete conformal parameterization with circle border
UV <- mesh$parameterization("DCP", UVborder = "circle")

# now we extract the colors from the image
library(imager)
# load the image
img <- load.image("FermatSpiral.png")
...
mesh$assignNormals(mesh$getVertices())
rmesh <- mesh$getMesh()
...
gifski(
  ...
  gif_file = "SphericalCap-FermatSpiral.gif",
  width = 512,
  height = 512,
  delay = 1/8
)
