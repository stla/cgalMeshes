...
# MM surface parameterization ####
MM <- Vectorize(function(r, t) {
  alpha <- r - 1/r
  beta  <- r*r + 1/(r*r)
  gamma <- (r*r*r - 1/(r*r*r))/3
  c(
    -alpha*sin(t) - beta*sin(2*t) - gamma*sin(3*t),
    alpha*cos(t) + beta*cos(2*t) + gamma*cos(3*t),
    -2 * alpha*sin(t)
  )
})
...
rmesh <- parametricMesh(
  MM, c(1, 2.5), c(0, 2*pi), c(FALSE, TRUE), nu = 512, nv = 512,
  clean = FALSE
)
...
mesh$isotropicRemeshing(0.05, iterations = 3, relaxSteps = 2)
