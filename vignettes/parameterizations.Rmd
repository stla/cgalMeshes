...
of a torus with a minor radius $r$ and a major radius $R=\sqrt{1+r^2}$:
...
# construct the mesh
torusMesh <- parametricMesh(
  torus, c(0, 2*pi), c(0, 2*pi), periodic = c(TRUE, TRUE),
  nu = 512L, nv = 512L, clean = TRUE
)
torusMesh <- addNormals(torusMesh)
# assign the checkerboard (the same as before) to the mesh
torusMesh$material <- list(color = checkerboard)
```
...
As you can see, the right angles are not distorted (an angle on a curved 
surface is the angle between the tangents). This is a conformal 
parameterization.
...
Now we construct a parametrization. I choose the 
*discrete conformal parameterization*. This one tries to be conformal, that is 
to say it minimizes the angles distortion.
...
planar domain of the $uv$-coordinates: a square or a circle. I take the circle 
here.
