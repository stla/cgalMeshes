...
Rcpp::List clippingToIsoCuboid(EMesh3& tm, IsoCuboid3 isocuboid, const bool clipVolume) {
  if(!CGAL::is_triangle_mesh(tm)) {
    Rcpp::stop("The mesh is not triangle.");
  }
  if(PMP::does_self_intersect(tm)) {
...
// [[Rcpp::export]]
Rcpp::XPtr<EMesh3> sphericalTriangle(
  Rcpp::XPtr<EMesh3> sphereMeshXPtr, Rcpp::NumericVector A, Rcpp::NumericVector B, Rcpp::NumericVector C
) {
  EMesh3 sphereMesh = *(sphereMeshXPtr.get());
  EPoint3 pa(A(0), A(1), A(2));
  EPoint3 pb(B(0), B(1), B(2));
  EPoint3 pc(C(0), C(1), C(2));
  EPoint3 O(0.0, 0.0, 0.0);
  EPlane3 Oab(O, pa, pb);
  if(!Oab.has_on_positive_side(pc)) {
    Oab = Oab.opposite();
  }
  EPlane3 Obc(O, pb, pc);
  if(!Obc.has_on_positive_side(pa)) {
    Obc = Obc.opposite();
  }
  EPlane3 Oca(O, pc, pa);
  if(!Oca.has_on_positive_side(pb)) {
    Oca = Oca.opposite();
  }

  Rcpp::List l1 = clippingToPlane(sphereMesh, Oab, false);
  Rcpp::List l2 = clippingToPlane(sphereMesh, Obc, false);
  Rcpp::List l3 = clippingToPlane(sphereMesh, Oca, false);
...
Rcpp::XPtr<EMesh3> icosphere(unsigned int iterations) {
  EMesh3 mesh;
  CGAL::make_icosahedron<EMesh3, EPoint3>(mesh);
